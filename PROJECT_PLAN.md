...
- [x] Use `decimal.Decimal` (shopspring/decimal) for all monetary/quantity fields.
...
Tasks:
- [x] Create `internal/adapters/binance` package implementing `VenueConnector`.
- [x] Implement REST client for `PlaceOrder`, `CancelOrder`, `GetOrder`, `GetFills`, `GetBalances` against `https://api.binance.com`.
- [x] Add Binance Edge auth: API key header, HMAC-SHA256 signing of query string with `timestamp` + `recvWindow`.
- [x] Map Binance response payloads to shared `VenueOrder`, `Fill`, `Balances` types.
- [x] Add WS client struct for `wss://stream.binance.com:9443/ws` (connection only; subscription wiring in Stage 5).
- [x] Wire `VENUE_FAMILY=binance` selection in `cmd/exchange-connectors`.
- [x] Add table-driven unit tests with recorded HTTP fixtures (httptest).
...
Tasks:
- [x] Create `internal/adapters/kraken` package implementing `VenueConnector`.
- [x] Implement REST client for `PlaceOrder`, `CancelOrder`, `GetOrder`, `GetFills`, `GetBalances` against `https://api.kraken.com`.
- [x] Add Kraken auth: API key header, SHA256+HMAC-SHA512 signing of path + nonce + POST data.
- [x] Map Kraken response payloads (incl. `result`/`error` envelope) to shared types.
- [x] Wire `VENUE_FAMILY=kraken` selection in `cmd/exchange-connectors`.
- [x] Add table-driven unit tests with recorded HTTP fixtures.
...
Tasks:
- [x] Create `internal/adapters/otc` package implementing `VenueConnector`.
- [x] Implement REST client for `PlaceOrder` (RFQ → quote → accept), `CancelOrder`, `GetOrder`, `GetFills`, `GetBalances` against `OTC_DESK_URL`.
- [x] Add `SubscribeOrderBook` returning `errors.ErrUnsupported` (no public market data).
- [x] Add SFTP fallback for settlement confirmations when configured.
- [x] Map OTC desk response payloads to shared types; normalize manual settlement states.
- [x] Wire `VENUE_FAMILY=otc` selection in `cmd/exchange-connectors`.
- [x] Add table-driven unit tests with recorded HTTP/SFTP fixtures.
...
Tasks:
- [x] Create `internal/ws` package with a generic reconnecting WS client (exponential backoff + jitter, bounded by `WS_RECONNECT_MAX_BACKOFF`).
- [x] Implement `SubscribeOrderBook` for Binance (diff depth stream + depth snapshot REST bootstrapping).
- [x] Implement `SubscribeOrderBook` for Kraken (`book` channel with depth snapshots).
- [x] Add sequence-gap detection: on gap, force a snapshot refresh and drop stale diffs.
- [x] Resubscribe to book + private channels after reconnect; emit `BookUpdate` to caller channel.
- [x] Add metrics: `ws_reconnect_count`, `ws_gap_count`, `ws_book_lag_seconds`.
...
Tasks:
- [x] Create `internal/ratelimit` package with a weighted token-bucket limiter per venue.
- [x] Binance: read `X-MBX-USED-WEIGHT-1M` response header; adjust budget; back off on 429/418.
- [x] Kraken: model the Kraken counter system (decay-based) and gate requests by counter budget.
- [x] OTC: simple RPS limit configurable via `RATE_LIMIT_WEIGHT_BUDGET`.
- [x] When budget is exhausted, queue or reject internal requests rather than risk a ban; surface `rate_limit_headroom` metric.
- [x] Add unit tests for budget accounting, header-driven adjustment, and 429 backoff.
...
Tasks:
- [x] Create `internal/secrets` package wrapping the platform vault client (HashiCorp Vault / cloud KMS).
- [x] Read keys from `secret/exchange-connectors/<venue>/api-key` and `.../api-secret` at startup; hold a lease.
- [x] Implement rotation via `POST /admin/rotate-credentials` and vault rotation signal.
- [x] Old keys revoked only after the connector confirms the new key is active (probe call).
- [x] Redact secrets from logs, API responses, and error messages; no disk persistence.
- [x] Add `venue_credentials` rows referencing vault secret versions only (no secret material).
- [x] Add unit tests with a fake vault client covering load, rotate, revoke, and redaction.
...
- [x] Propagate traces from Liquidity Routing through the connector to the audit call.
- [x] Make audit emission non-blocking with bounded queue + drop-on-overflow policy (log + metric on drops).
- [x] Add metric `audit_events_emitted_total` and `audit_dropped_total`.
